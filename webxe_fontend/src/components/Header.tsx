...
import { HeaderTopAnimation } from './WelcomeAnimation';
...
export default function Header() {
  ...
  useEffect(() => {
    if (status !== 'loading' && headerTopRef.current && !animationRun.current) {
      animationRun.current = true;

      const shouldShow = typeof window !== 'undefined' ? sessionStorage.getItem('showWelcomeAnimation') : null;
      if (shouldShow === 'true' && isAuthenticated) {
        sessionStorage.removeItem('showWelcomeAnimation');
        const username = user?.name ? user.name : (user?.username ? user.username : 'Khách');
        new HeaderTopAnimation(headerTopRef.current, username);
      }
    }
